              ...
              std::vector<Quad *> & quadlist)
{
    ...
    for (unsigned int i=0;i<16;i++){
        ...
        std::cout<<"Quad "<<quadlist.back()->id()<<
                ...
                quadlist.back()->n3()->id()<<","<<
                quadlist.back()->n4()->id()<<"]"<<
                "area is "<<quadlist.back()->area()<<std::endl;
    }
        for(size_t i=0;i<16;i++)
    {
        ...
        {
            if(i!=k){
            if(quadlist[i]->isneighbor(quadlist[k]))
               {
                   quadlist[i]->neighbors().push_back(quadlist[k]);
                    std::cout<<"Quad "<<quadlist[i]->id()<<
                    " has neighbor "<<"Quad "<<quadlist[i]->neighbors().back()->id()<<std::endl;
               }
            }
        }
    }
